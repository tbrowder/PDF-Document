...
if not @*ARGS {
    ...
    Creates a simple check at the top of a US Letter page.
    HERE
    exit;
}
for @*ARGS {
...
# We change only two of the many defaults for this
# example: (1) output file name and (2) force option to
# allow overwriting that file if it exists.
my \d = Doc.new: :pdf-name<example-check>, :force, :$debug;
