...
=item Define the C<PDF> class instance (a heavy-weight instanciation, only one per document)
...
Currently the only thing the module provides are information to ease
use of PDF core fonts (those shown in the listing above) in a using module.
