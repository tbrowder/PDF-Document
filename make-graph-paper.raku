...
# title of output pdf
my $mx = 0.5; # default x margins, inches
my $my = 0.5; # default y margins, inches

my $cx = 0.5; # cell width as proportion of paper width less margins
my $cy = 0.5; # cell height as proportion of paper width less margins
my $ofile = "graph-paper.pdf";

say qq:to/HERE/;
OBSOLETE See my module 'PDF::GraphPaper'.
HERE
...
if not @*ARGS.elems {
    ...
    Produces graph paper as a PDF document.

    Also installs a config file '$HOME/.pdf-graph-paper' for default
    options which may be changed by the user. Any existing file will not
    be modified by an upgrade.

    Options (overide the config file):
